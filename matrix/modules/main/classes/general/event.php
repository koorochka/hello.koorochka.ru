...
use Matrix\Main\Mail;

IncludeModuleLangFile(__FILE__);

global $MX_EVENT_SITE_PARAMS;
$MX_EVENT_SITE_PARAMS = array();
...
class CAllEvent
{
	...
	{
		global $MX_EVENT_SITE_PARAMS;
		if($site_id !== false && isset($MX_EVENT_SITE_PARAMS[$site_id]))
			return $MX_EVENT_SITE_PARAMS[$site_id];
		...
		if(strlen($site_id)>0)
		{
			...
			{
				$MX_EVENT_SITE_PARAMS[$site_id] = array(
	...
	/**
	 * @deprecated See \Matrix\Main\Mail\Mail::is8Bit()
	 */
	...
	/**
	 * @deprecated See \Matrix\Main\Mail\Mail::encodeMimeString()
	 */
	...
	/**
	 * @deprecated See \Matrix\Mail\Mail::encodeSubject()
	 */
	...
	/**
	 * @deprecated See \Matrix\Main\Mail\Mail::encodeHeaderFrom()
	 */
	...
	/**
	 * @deprecated See \Matrix\Main\Mail\Mail::getMailEol()
	 */
	...
	/**
	 * @deprecated See \Matrix\Main\Mail\Event::handleEvent()
	 */
	...
	public function Add($arFields)
	{
		...
		if ($result->isSuccess())
		{
			...
			if(count($arLID)>0)
			{
				...
				$resultDb = \Matrix\Main\SiteTable::getList(array(
					'select' => array('LID'),
		...
		$resultDb = Mail\Internal\EventMessageTable::getList(array(
			...
			'runtime' => array(
				'EVENT_MESSAGE_TYPE' => array(
					'data_type' => 'Matrix\Main\Mail\Internal\EventType',
					'reference' => array('=this.EVENT_NAME' => 'ref.EVENT_NAME', '=ref.LID' => new \Matrix\Main\DB\SqlExpression('?', LANGUAGE_ID)),
				),
			)
		));
...
class CEventType
{
	...
	public static function GetListEx($arOrder = array(), $arFilter = array(), $arParams = array())
	{
		...
		$query1 = new \Matrix\Main\Entity\Query(Mail\Internal\EventMessageTable::getEntity());
		...
		$query2 = new \Matrix\Main\Entity\Query(Mail\Internal\EventTypeTable::getEntity());
		...
		$query2->registerRuntimeField('EVENT_MESSAGE', array(
			'data_type' => 'Matrix\Main\Mail\Internal\EventMessage',
			'reference' => array('=this.EVENT_NAME' => 'ref.EVENT_NAME'),
		));

		$connection = \Matrix\Main\Application::getConnection();
