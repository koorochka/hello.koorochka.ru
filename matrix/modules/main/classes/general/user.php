 ...
 * @package matrix
 * @subpackage main
 * @copyright 2001-2013 Matrix
 */

use Matrix\Main;
use Matrix\Main\Authentication\ApplicationPasswordTable;

IncludeModuleLangFile(__FILE__);

global $MX_GROUP_POLICY;
$MX_GROUP_POLICY = array(
...
abstract class CAllUser extends CDBResult
{
	...
	public function RequiredHTTPAuthBasic($Realm = "Matrix")
	{
	...
	public function LoginByDigest($arDigest)
	{
		...
		if($arUser = $res->Fetch())
		{
			...
			if($arUser["EXTERNAL_AUTH_ID"] == '' && $arUser["DIGEST_HA1"] <> '')
			{
				...
				$_SESSION["MX_HTTP_DIGEST_ABSENT"] = false;
			...
			if(($appPassword = \Matrix\Main\Authentication\ApplicationPasswordTable::findDigestPassword($arUser["ID"], $arDigest)) !== false)
			{
			...
			if($arUser["DIGEST_HA1"] == '')
			{
				...
				$_SESSION["MX_HTTP_DIGEST_ABSENT"] = true;
			}
		}
	...
	public static function UpdateDigest($ID, $pass)
	{
		...
		if($arRes = $res->Fetch())
		{
			if(defined('MX_HTTP_AUTH_REALM'))
				$realm = MX_HTTP_AUTH_REALM;
			else
				$realm = "Matrix Site Manager";
	...
	protected function UpdateSessionData($id, $applicationId = null)
	{
		...
		$_SESSION["MX_LOGIN_NEED_CAPTCHA"] = false;
		...
		if($arUser = $result->Fetch())
		{
			...
			$_SESSION["SESS_AUTH"]["MX_USER_ID"] = $arUser["MX_USER_ID"];
	...
	public function Authorize($id, $bSave = false, $bUpdate = true, $applicationId = null)
	{
		...
		if($arUser !== false)
		{
			...
			if($bUpdate)
			{
				...
				$bxUid = '';
				if (!empty($_COOKIE['MX_USER_ID']) && preg_match('/^[0-9a-f]{32}$/', $_COOKIE['MX_USER_ID']))
				{
					if ($_COOKIE['MX_USER_ID'] != $arUser['MX_USER_ID'])
					{
						// save new bxuid value
						$bxUid = ", MX_USER_ID = '".$_COOKIE['MX_USER_ID']."'";

						$arUser['MX_USER_ID'] = $_COOKIE['MX_USER_ID'];
						$_SESSION["SESS_AUTH"]["MX_USER_ID"] = $_COOKIE['MX_USER_ID'];
					}
				}
				...
				if($applicationId === null && ($bSave || COption::GetOptionString("main", "auth_multisite", "N") == "Y"))
				{
					...
					if($bSave)
					{
						...
						$spread = MX_SPREAD_SITES | MX_SPREAD_DOMAIN;
					}
					else
					{
						$period = 0;
						$spread = MX_SPREAD_SITES;
					}
		...
		if($bOk)
		{
			...
			if($user_id <= 0)
			{
				...
				if(($arUser = $result->Fetch()))
				{
					...
					{
						$_SESSION["MX_LOGIN_NEED_CAPTCHA"] = true;
		...
		if($user_id > 0)
		{
			if($applicationId === null && CModule::IncludeModule("security"))
			{
				...
				*/
				...
				$doAuthorize = \Matrix\Security\Mfa\Otp::verifyUser($arParams);
			}
	...
	{
		if(!CModule::IncludeModule("security") || !\Matrix\Security\Mfa\Otp::isOtpRequired())
		{
			return array("MESSAGE" => GetMessage("USER_LOGIN_OTP_ERROR")."<br>", "TYPE" => "ERROR");
		}

		$userParams = \Matrix\Security\Mfa\Otp::getDeferredParams();
		...
		if(!\Matrix\Security\Mfa\Otp::verifyUser($userParams))
		{
	...
	public function AuthorizeWithOtp($user_id)
	{
		...
		if(CModule::IncludeModule("security"))
		{
			...
			When authorization is disallowed the OTP form will be shown on the next hit.
			*/
			$doAuthorize = \Matrix\Security\Mfa\Otp::verifyUser(array("USER_ID" => $user_id));
		}
	...
	public function Register($USER_LOGIN, $USER_NAME, $USER_LAST_NAME, $USER_PASSWORD, $USER_CONFIRM_PASSWORD, $USER_EMAIL, $SITE_ID = false, $captcha_word = "", $captcha_sid = 0, $bSkipConfirm = false)
	{
		...
		{
			$agreementObject = new \Matrix\Main\UserConsent\Agreement($agreementId);
			if ($agreementObject->isExist() && $agreementObject->isActive() && $_REQUEST["USER_AGREEMENT"] == "Y")
			{
				\Matrix\Main\UserConsent\Consent::addByContext($agreementId, "main/reg", "register");
			}
		}
	...
	public static function SetLastActivityDateByArray($arUsers)
	{
		...
		$event = new \Matrix\Main\Event("main", "OnUserSetLastActivityDate", array($arUsers));
		$event->send();
	...
	{
		return \Matrix\Main\UserTable::getSecondsForLimitOnline();
	}
	...
	public static function GetOnlineStatus($userId, $lastseen, $now = false)
	{
		...
		if ($lastseen instanceof \Matrix\Main\Type\DateTime)
		{
		...
		else if ($now instanceof \Matrix\Main\Type\DateTime)
		{
	...
	/**
	 * @param int|bool|\Matrix\Main\Type\DateTime $timestamp
	 * @param int|bool|\Matrix\Main\Type\DateTime $now
	 * 
	 * @return string
	 */
	public static function FormatLastActivityDate($timestamp, $now = false)
	{
		...
		if ($timestamp instanceof \Matrix\Main\Type\DateTime)
		{
